//! Rust dead-import adapter: rustc-certified `unused_imports` via an isolated
//! `cargo check --message-format=json` + the `rustfix` crate.
//!
//! Deliberately *not* `cargo fix`, which also applies edition and `dyn` rewrites
//! that are out of scope. The JSON stream is parsed (never regex on stderr) and
//! filtered to the single `unused_imports` lint class before `rustfix` applies the
//! `MachineApplicable` deletions. See the module docs in [`super`] for the shared
//! engine that isolates the copy and maps paths back to the real repo.
...
/// The R0 eligibility precondition for the Rust dead-import fix. Several of these
/// are **declared, not enforced** — stating them honestly (rather than pretending
/// to validate) is the point: a campaign operator pre-registers that the target
/// repo class satisfies them.
...
/// Removes rustc-certified unused imports via an isolated `cargo check` + `rustfix`.
pub(crate) struct RustImportAdapter;

impl ImportAdapter for RustImportAdapter {
    ...
    fn is_eligible(&self, repo: &Path) -> bool {
        // Cargo-only treatment: a tree without a root manifest is ineligible.
...
/// Run `cargo check` over the isolated copy and return its parsed JSON diagnostics.
...
/// Build the per-file subtractive rewrites from the parsed compiler diagnostics:
/// keep only `unused_imports` `MachineApplicable` suggestions, apply them per
/// file, and return each touched file's tree-relative path plus its fixed content.
/// The shared engine maps these back to the real repo.
fn compute_subtracted(
    diagnostics: &[Value],
    work: &Path,
    ...
    // Canonicalize the work root once so absolute rustc spans resolve correctly
    // even when /tmp is a symlink.
...
#[cfg(test)]
mod tests {
    use super::super::{finalize_changes_for_test, DeadImportFix};
    use super::*;
    ...
    /// [`compute_subtracted`] and [`classify_build`] consume.
    ...
    fn compute_changes_drops_a_certified_unused_import() {
        ...
        let subtracted = compute_subtracted(&diags(&json), work.path()).unwrap();
        let changes = finalize_changes_for_test(subtracted, work.path(), repo.path()).unwrap();
    ...
    fn compute_changes_filters_out_non_unused_import_machine_applicable_suggestions() {
        ...
        let subtracted = compute_subtracted(&parsed, work.path()).unwrap();
        let changes = finalize_changes_for_test(subtracted, work.path(), repo.path()).unwrap();
        assert!(
            changes.is_empty(),
    ...
    fn provenance_records_toolchain_and_pin_presence() {
        ...
        let unpinned = RustImportAdapter.provenance(repo.path()).unwrap().unwrap();
        ...
        .unwrap();
        let pinned = RustImportAdapter.provenance(repo.path()).unwrap().unwrap();
