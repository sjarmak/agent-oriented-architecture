//! Metric-extractor substrate types (R15 tiering, gold/invariant anchoring).
//!
//! These types are the shared input surface consumed by `aoa-metrics`'
//! extractors and produced by graph producers such as `aoa-scip-graph`. They
//! live here (rather than in `aoa-metrics`) because graph producers only need
//! to *name* their own output type, not to link the locality-math extractors
//! that consume it — hosting them in the substrate crate every crate already
//! depends on keeps that dependency one-directional.
//!
//! `aoa-metrics` re-exports these under their original paths for back-compat
//! (`aoa_metrics::SymbolGraph` etc. still resolve).
