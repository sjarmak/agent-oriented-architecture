...
This repo dogfoods its own runtime enforcement plane: the tracked `.claude/settings.json` registers the five Claude Code hooks that `aoa observe --enforce` installs, covering `record` (PostToolUse on Bash), `check` (PreToolUse on the mutation tools), and one hook per write outcome. Every one of them runs the tracked wrapper `.claude/hooks/aoa-enforce`, which the same command installs. The check hook enforces the reproduction-before-mutation gate (R7): a Write or Edit with no preceding test run is blocked. To run without the gate, set `reproduction_required: false` in an untracked `aoa-policy.yaml` at the repo root.

Every one of those policies is about this repository, so the gate's path domain is this repository. The hook matcher fires on `Write|Edit|MultiEdit|NotebookEdit` and cannot express paths, so `aoa enforce check` resolves the target itself and returns allow, ungated and without recording a span, for anything landing outside the repo root — a session working in this checkout can still write notes, scratch files, and reports elsewhere on the machine without first running a test here (aoa-7g14y.1). Containment is decided on the resolved path, so a `../` or symlinked spelling that walks back into the repo is still gated; a repo-local symlink pointing out of the tree is still gated too, since otherwise the alias would be a way to spell a protected path and have the gate wave it through.

The wrapper resolves the binary itself, in order: `$AOA_BIN`, then `aoa` on PATH, then this checkout's `target/release/aoa` and `target/debug/aoa`. Earlier versions registered a bare `aoa enforce <verb>` instead, which only ran where the binary happened to be on the PATH the host handed the hook — and where it was not, the hooks failed non-blocking, the host swallowed the error, and the plane read as installed while never running once (aoa-zsem6). The wrapper exists so that state is impossible to reach silently: when it cannot find a binary it says so on stderr, and the blocking `check` hook denies the write rather than waving it through.
