...
The wrapper resolves the binary itself, in order: `$AOA_BIN`, then `aoa` on PATH, then this checkout's `target/release/aoa` and `target/debug/aoa`. Earlier versions registered a bare `aoa enforce <verb>` instead, which only ran where the binary happened to be on the PATH the host handed the hook — and where it was not, the hooks failed non-blocking, the host swallowed the error, and the plane read as installed while never running once (aoa-zsem6). The wrapper exists so that state is impossible to reach silently: when it cannot find a binary it says so on stderr, and the blocking `check` hook denies the write rather than waving it through. `aoa observe --enforce` retires the old bare commands when it upgrades a repo.
