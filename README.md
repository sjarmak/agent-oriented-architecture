...
This repo dogfoods its own runtime enforcement plane: the tracked `.claude/settings.json` registers the two Claude Code hooks that `aoa observe --enforce` installs, `aoa enforce record` (PostToolUse on Bash) and `aoa enforce check` (PreToolUse on the mutation tools). With `aoa` on your PATH (`cargo install --path crates/aoa`), the check hook enforces the reproduction-before-mutation gate (R7): a Write or Edit with no preceding test run is blocked. Without `aoa` installed the hooks fail as non-blocking errors on each guarded tool call; install the binary, or keep the hooks while relaxing the gate by setting `reproduction_required: false` in an untracked `aoa-policy.yaml` at the repo root.
